      ...
      {showForm && (
        <form onSubmit={handleFormSubmit} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 space-y-4 max-w-lg mx-auto">
          <div className="flex justify-between items-center mb-2">
            <span className="font-semibold text-lg">{editId ? 'Edit Transaction' : 'New Transaction'}</span>
            {editId && (
            ...
            {filtered.map((tx, index) => (
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-gray-500">{tx.category} • {tx.type} • {new Date(tx.date).toLocaleDateString()}</div>
                </div>
                <div className="flex items-center space-x-4">
